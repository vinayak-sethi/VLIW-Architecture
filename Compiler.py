...
class Compiler:
        ...
        for x in fileData:
            ...
            if j[0] in ["10010", "10011", "10100"]:
                ...
                if x.find("#") != -1:
                    ...
                    flag = True
            self.data.append(j)
        print('\x1b[6;31;49m'+f"Total Number of Errors: {errorCount}\n"+'\x1b[0m')
        if flag:
            exit()
    ...
    def __createPackets(self):
        ...
        check_waw = False
        check_raw = False
        # Loop through all the data from the .s file
        for data in self.data:
            # Get the keys for the data
            packetKey = getPacketKey(data[0])
            ...
            for packet in self.packets:
                ...
                if not check_war and not check_waw and not check_raw:
                    flag = addInst(packet, packetKey, data)
                    if flag:
                        break
